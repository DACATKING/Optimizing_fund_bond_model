...
class NeuralNetHelper:
    ...
    def train_minibatch(self, epochs=parameters['epoch'], device="cpu", random_seed_=None):
...
#            for inputs, labels in tqdm(nn_dataset_loader)
            for inputs, labels in nn_dataset_loader:
                # clear the grad
...
#            log_message("{} training loss average compared to benchmark: {}".format(epoch + 1, full_loss / counter))
            ...
            epoch_time = time.time() - starts_time  # Calculate elapsed time
